...
- **RG58 coax, braided shield only.** One continuous length, ~5–6 m, so a single run reaches from the
  antenna all the way to the radio. **Do not use foil-shielded coax**, as the foil cracks at the sharp
...
| --- | --- | --- |
| **Top element (radiator)** | **457 mm** | Strip off the outer sheath **and** braid, exposing the centre conductor only |
